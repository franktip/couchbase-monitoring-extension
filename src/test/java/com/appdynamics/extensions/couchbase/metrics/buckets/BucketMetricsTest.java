 ...
 public class BucketMetricsTest {
     ...
     public void overallBucketMetricsTest() throws IOException{
         try (MockedStatic<MetricPathUtils> mocked =  Mockito.mockStatic(MetricPathUtils.class)) {
             ...
             // when(MetricPathUtils.buildMetricPath(anyString(),anyString())).thenReturn("Custom Metrics|CouchBase|Cluster1|buckets|beer-sample");
             mocked.when(() -> MetricPathUtils.buildMetricPath(Mockito.anyString(),Mockito.anyString())).thenReturn("Custom Metrics|CouchBase|Cluster1|buckets|beer-sample");
